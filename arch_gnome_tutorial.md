...
all of them are in my opinion essential to the well functioning of a desktop except for kitty and pkgstats. 
...
pacman -S --needed \
  ...
  gnome-shell gnome-session gnome-shell-extensions gnome-shell-extension-appindicator \
  gnome-shell-extension-desktop-icons-ng gnome-shell-extension-dash-to-panel gnome-shell-extension-arc-menu \
  gnome-shell-extension-weather-oclock extension-manager gnome-control-center gnome-terminal kitty \
  gdm nautilus linux-zen-headers linux-lts-headers xdg-desktop-portal-gnome xdg-user-dirs-gtk \
  nvidia-open-dkms nvidia-utils libva-nvidia-driver libva-utils cuda \
  loupe papers gnome-calendar gnome-system-monitor gnome-keyring gnome-logs gnome-clocks gnome-weather \
  gvfs gvfs-smb gvfs-wsdd gvfs-nfs gvfs-dnssd gvfs-mtp gvfs-gphoto2 gvfs-afc gvfs-google gvfs-onedrive \
  gvfs-goa snapshot grilo-plugins gnome-menus gnome-backgrounds gnome-tweaks gnome-epub-thumbnailer \
...
systemctl enable NetworkManager gdm.service systemd-timesyncd systemd-boot-update.service \
...
Head to `arch_gnome_post_tutorial.md` to do the post-install tutorial. (WIP)
